// Code generated by `ubx sdk gen` from cloudflare_access_scim_users_response. DO NOT EDIT.
import type { Computed, FieldMap, DataSourceBinding } from "@ubx/sdk";

export interface AccessScimUsersResponse_Result_Emails {
  primary?: boolean | Computed<boolean>;
  type?: string | Computed<string>;
  value?: string | Computed<string>;
}

export interface AccessScimUsersResponse_Result_Meta {
  created?: string | Computed<string>;
  lastModified?: string | Computed<string>;
}

export interface AccessScimUsersResponse_Result {
  active?: boolean | Computed<boolean>;
  displayName?: string | Computed<string>;
  emails?: AccessScimUsersResponse_Result_Emails[] | Computed<AccessScimUsersResponse_Result_Emails[]>;
  externalId?: string | Computed<string>;
  id?: string | Computed<string>;
  meta?: AccessScimUsersResponse_Result_Meta | Computed<AccessScimUsersResponse_Result_Meta>;
  schemas?: string[] | Computed<string[]>;
}

export interface AccessScimUsersResponseConfig {
  /** Identifier. */
  accountId: string | Computed<string>;
  /** The unique Cloudflare-generated Id of the SCIM User resource; also known as the "Id". Pass once for a single lookup (`?cf_resource_id=A`) or repeat the parameter (`?cf_resource_id=A&cf_resource_id=B`) to look up multiple users in one request, up to 50 values. Mutually exclusive with `idp_resource_id`, `username`, `email`, `name`, `search_contains`, and `search_starts_with`. */
  cfResourceId?: string[] | Computed<string[]>;
  /** The email address of the SCIM User resource. */
  email?: string | Computed<string>;
  /** UUID. */
  identityProviderId: string | Computed<string>;
  /** The IdP-generated Id of the SCIM User resource; also known as the "external Id". Pass once for a single lookup (`?idp_resource_id=A`) or repeat the parameter (`?idp_resource_id=A&idp_resource_id=B`) to look up multiple users in one request, up to 50 values. Mutually exclusive with `cf_resource_id`, `username`, `email`, `name`, `search_contains`, and `search_starts_with`. */
  idpResourceId?: string[] | Computed<string[]>;
  /** The name of the SCIM User resource. */
  name?: string | Computed<string>;
  /** Page number of results. */
  page?: number | Computed<number>;
  /** Number of results per page. */
  perPage?: number | Computed<number>;
  /** The username of the SCIM User resource. */
  username?: string | Computed<string>;
}

export interface AccessScimUsersResponseAttrs {
  /** Identifier. */
  accountId: string;
  /** The unique Cloudflare-generated Id of the SCIM User resource; also known as the "Id". Pass once for a single lookup (`?cf_resource_id=A`) or repeat the parameter (`?cf_resource_id=A&cf_resource_id=B`) to look up multiple users in one request, up to 50 values. Mutually exclusive with `idp_resource_id`, `username`, `email`, `name`, `search_contains`, and `search_starts_with`. */
  cfResourceId: string[];
  /** The email address of the SCIM User resource. */
  email: string;
  /** UUID. */
  identityProviderId: string;
  /** The IdP-generated Id of the SCIM User resource; also known as the "external Id". Pass once for a single lookup (`?idp_resource_id=A`) or repeat the parameter (`?idp_resource_id=A&idp_resource_id=B`) to look up multiple users in one request, up to 50 values. Mutually exclusive with `cf_resource_id`, `username`, `email`, `name`, `search_contains`, and `search_starts_with`. */
  idpResourceId: string[];
  /** The name of the SCIM User resource. */
  name: string;
  /** Page number of results. */
  page: number;
  /** Number of results per page. */
  perPage: number;
  result: AccessScimUsersResponse_Result[];
  /** The username of the SCIM User resource. */
  username: string;
}

export const AccessScimUsersResponse: DataSourceBinding<AccessScimUsersResponseConfig, AccessScimUsersResponseAttrs> = {
  wireType: "cloudflare_access_scim_users_response",
  fields: {
    accountId: "account_id",
    cfResourceId: "cf_resource_id",
    email: "email",
    identityProviderId: "identity_provider_id",
    idpResourceId: "idp_resource_id",
    name: "name",
    page: "page",
    perPage: "per_page",
    username: "username",
  },
};
