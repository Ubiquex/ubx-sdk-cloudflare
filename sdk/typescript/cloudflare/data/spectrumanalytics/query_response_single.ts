// Code generated by `ubx sdk gen` from cloudflare_spectrum_analytics_query_response_single. DO NOT EDIT.
import type { Computed, FieldMap, DataSourceBinding } from "@ubx/sdk";

export interface QueryResponseSingle_Result_Data {
  dimensions?: string[] | Computed<string[]>;
  metrics?: unknown | Computed<unknown>;
}

export interface QueryResponseSingle_Result_Max {
}

export interface QueryResponseSingle_Result_Query {
  /** Can be used to break down the data by given attributes. Options are: Dimension | Name | Example --------------------------|---------------------------------|-------------------------- event | Connection Event | connect, progress, disconnect, originError, clientFiltered appID | Application ID | 40d67c87c6cd4b889a4fd57805225e85 coloName | Colo Name | SFO ipVersion | IP version used by the client | 4, 6. */
  dimensions?: string[] | Computed<string[]>;
  /** Used to filter rows by one or more dimensions. Filters can be combined using OR and AND boolean logic. AND takes precedence over OR in all the expressions. The OR operator is defined using a comma (,) or OR keyword surrounded by whitespace. The AND operator is defined using a semicolon (;) or AND keyword surrounded by whitespace. Note that the semicolon is a reserved character in URLs (rfc1738) and needs to be percent-encoded as %3B. Comparison options are: Operator | Name | URL Encoded --------------------------|---------------------------------|-------------------------- == | Equals | %3D%3D != | Does not equals | !%3D \> | Greater Than | %3E \< | Less Than | %3C \>= | Greater than or equal to | %3E%3D \<= | Less than or equal to | %3C%3D Use the above to construct filters. */
  filters?: string | Computed<string>;
  /** Limit number of returned metrics. */
  limit?: number | Computed<number>;
  /** One or more metrics to compute. Options are: Metric | Name | Example | Unit --------------------------|-------------------------------------|--------------------------|-------------------------- count | Count of total events | 1000 | Count bytesIngress | Sum of ingress bytes | 1000 | Sum bytesEgress | Sum of egress bytes | 1000 | Sum durationAvg | Average connection duration | 1.0 | Time in milliseconds durationMedian | Median connection duration | 1.0 | Time in milliseconds duration90th | 90th percentile connection duration | 1.0 | Time in milliseconds duration99th | 99th percentile connection duration | 1.0 | Time in milliseconds. */
  metrics?: string[] | Computed<string[]>;
  since?: QueryResponseSingle_Result_Max | Computed<QueryResponseSingle_Result_Max>;
  /** The sort order for the result set; sort fields must be included in `metrics` or `dimensions`. */
  sort?: string[] | Computed<string[]>;
  until?: QueryResponseSingle_Result_Max | Computed<QueryResponseSingle_Result_Max>;
}

export interface QueryResponseSingle_Result {
  /** List of columns returned by the analytics query. */
  data: QueryResponseSingle_Result_Data[] | Computed<QueryResponseSingle_Result_Data[]>;
  /** Number of seconds between current time and last processed event, i.e. how many seconds of data could be missing. */
  dataLag: number | Computed<number>;
  max: QueryResponseSingle_Result_Max | Computed<QueryResponseSingle_Result_Max>;
  min: QueryResponseSingle_Result_Max | Computed<QueryResponseSingle_Result_Max>;
  query: QueryResponseSingle_Result_Query | Computed<QueryResponseSingle_Result_Query>;
  /** Total number of rows in the result. */
  rows: number | Computed<number>;
  /** List of time interval buckets: [start, end]. */
  timeIntervals?: string[][] | Computed<string[][]>;
  totals: QueryResponseSingle_Result_Max | Computed<QueryResponseSingle_Result_Max>;
}

const QueryResponseSingle_Result_MaxFields: FieldMap = {
};

export interface QueryResponseSingleConfig {
  /** Can be used to break down the data by given attributes. Options are: Dimension | Name | Example --------------------------|---------------------------------|-------------------------- event | Connection Event | connect, progress, disconnect, originError, clientFiltered appID | Application ID | 40d67c87c6cd4b889a4fd57805225e85 coloName | Colo Name | SFO ipVersion | IP version used by the client | 4, 6. */
  dimensions?: string[] | Computed<string[]>;
  /** Used to filter rows by one or more dimensions. Filters can be combined using OR and AND boolean logic. AND takes precedence over OR in all the expressions. The OR operator is defined using a comma (,) or OR keyword surrounded by whitespace. The AND operator is defined using a semicolon (;) or AND keyword surrounded by whitespace. Note that the semicolon is a reserved character in URLs (rfc1738) and needs to be percent-encoded as %3B. Comparison options are: Operator | Name | URL Encoded --------------------------|---------------------------------|-------------------------- == | Equals | %3D%3D != | Does not equals | !%3D \> | Greater Than | %3E \< | Less Than | %3C \>= | Greater than or equal to | %3E%3D \<= | Less than or equal to | %3C%3D Use the above to construct filters. */
  filters?: string | Computed<string>;
  /** One or more metrics to compute. Options are: Metric | Name | Example | Unit --------------------------|-------------------------------------|--------------------------|-------------------------- count | Count of total events | 1000 | Count bytesIngress | Sum of ingress bytes | 1000 | Sum bytesEgress | Sum of egress bytes | 1000 | Sum durationAvg | Average connection duration | 1.0 | Time in milliseconds durationMedian | Median connection duration | 1.0 | Time in milliseconds duration90th | 90th percentile connection duration | 1.0 | Time in milliseconds duration99th | 99th percentile connection duration | 1.0 | Time in milliseconds. */
  metrics?: string[] | Computed<string[]>;
  since?: QueryResponseSingle_Result_Max | Computed<QueryResponseSingle_Result_Max>;
  /** The sort order for the result set; sort fields must be included in `metrics` or `dimensions`. */
  sort?: string[] | Computed<string[]>;
  /** Used to select time series resolution. */
  timeDelta: string | Computed<string>;
  until?: QueryResponseSingle_Result_Max | Computed<QueryResponseSingle_Result_Max>;
  /** Identifier. */
  zoneId: string | Computed<string>;
}

export interface QueryResponseSingleAttrs {
  /** Can be used to break down the data by given attributes. Options are: Dimension | Name | Example --------------------------|---------------------------------|-------------------------- event | Connection Event | connect, progress, disconnect, originError, clientFiltered appID | Application ID | 40d67c87c6cd4b889a4fd57805225e85 coloName | Colo Name | SFO ipVersion | IP version used by the client | 4, 6. */
  dimensions: string[];
  /** Used to filter rows by one or more dimensions. Filters can be combined using OR and AND boolean logic. AND takes precedence over OR in all the expressions. The OR operator is defined using a comma (,) or OR keyword surrounded by whitespace. The AND operator is defined using a semicolon (;) or AND keyword surrounded by whitespace. Note that the semicolon is a reserved character in URLs (rfc1738) and needs to be percent-encoded as %3B. Comparison options are: Operator | Name | URL Encoded --------------------------|---------------------------------|-------------------------- == | Equals | %3D%3D != | Does not equals | !%3D \> | Greater Than | %3E \< | Less Than | %3C \>= | Greater than or equal to | %3E%3D \<= | Less than or equal to | %3C%3D Use the above to construct filters. */
  filters: string;
  /** One or more metrics to compute. Options are: Metric | Name | Example | Unit --------------------------|-------------------------------------|--------------------------|-------------------------- count | Count of total events | 1000 | Count bytesIngress | Sum of ingress bytes | 1000 | Sum bytesEgress | Sum of egress bytes | 1000 | Sum durationAvg | Average connection duration | 1.0 | Time in milliseconds durationMedian | Median connection duration | 1.0 | Time in milliseconds duration90th | 90th percentile connection duration | 1.0 | Time in milliseconds duration99th | 99th percentile connection duration | 1.0 | Time in milliseconds. */
  metrics: string[];
  result: QueryResponseSingle_Result;
  since: QueryResponseSingle_Result_Max;
  /** The sort order for the result set; sort fields must be included in `metrics` or `dimensions`. */
  sort: string[];
  /** Used to select time series resolution. */
  timeDelta: string;
  until: QueryResponseSingle_Result_Max;
  /** Identifier. */
  zoneId: string;
}

export const QueryResponseSingle: DataSourceBinding<QueryResponseSingleConfig, QueryResponseSingleAttrs> = {
  wireType: "cloudflare_spectrum_analytics_query_response_single",
  fields: {
    dimensions: "dimensions",
    filters: "filters",
    metrics: "metrics",
    since: {
      wireName: "since",
      kind: "object",
      fields: QueryResponseSingle_Result_MaxFields,
    },
    sort: "sort",
    timeDelta: "time_delta",
    until: {
      wireName: "until",
      kind: "object",
      fields: QueryResponseSingle_Result_MaxFields,
    },
    zoneId: "zone_id",
  },
};
